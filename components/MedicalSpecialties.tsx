...
const specialties = [
  ...
  },
  {
    ...
    image: 'https://images.unsplash.com/photo-1545246651-07d28cc2a3f1?auto=format&fit=crop&w=600&q=80'
  },
  {
    id: 6,
...
function SpecialtyCard({ specialty, index }: { specialty: typeof specialties[0], index: number }) {
  ...
  return (
    <motion.div
      ...
      className="h-full"
    >
      <Card className="h-full hover:border-primary hover:shadow-2xl transition-all duration-300 group cursor-pointer bg-card/90 backdrop-blur-sm overflow-hidden">
        {/* Image Header */}
        <div className="relative h-48 overflow-hidden">
          ...
          />
          {/* Gradient overlay */}
          <div className={`absolute inset-0 bg-gradient-to-br ${specialty.gradient} opacity-60 group-hover:opacity-50 transition-opacity duration-300`} />
          
          {/* Icon on image */}
          <motion.div 
            className="absolute top-4 right-4 w-12 h-12 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center shadow-lg"
        ...
        <div className={`absolute inset-0 bg-gradient-to-br ${specialty.gradient} opacity-0 group-hover:opacity-5 transition-opacity duration-300 pointer-events-none`}></div>
      </Card>
    </motion.div>
  )
}
...
export default function MedicalSpecialties() {
  ...
  return (
    <section id="services" className="section py-24 px-6 sm:px-8 lg:px-12 bg-background relative overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 w-80 h-80 bg-accent/20 rounded-full blur-3xl opacity-10 -z-10"></div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <p className="text-primary font-semibold uppercase tracking-wide mb-3 text-sm">Our Services</p>
          <motion.h2 
